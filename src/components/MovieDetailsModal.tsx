...
export default function MovieDetailsModal({ movie, onClose }: MovieDetailsModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fade-in">
      ...
      {/* Modal Card */}
      <div className="relative w-full max-w-lg rounded-2xl border border-white/10 bg-[#181526] p-6 shadow-2xl z-50 animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto amethyst-scrollbar">
        <button
          onClick={onClose}
        ...
        <div className="relative w-full h-48 sm:h-56 rounded-xl overflow-hidden mb-5 border border-white/5 shadow-inner">
          <Image
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-t from-[#181526] via-[#181526]/25 to-black/30" />

          {/* Play Trailer Button overlay */}
          {movie.trailerUrl && (
            <button
              onClick={() => setShowTrailer(true)}
              className="absolute inset-0 flex items-center justify-center m-auto bg-purple-600 text-white rounded-full p-4 w-14 h-14 border border-purple-500 hover:scale-105 active:scale-95 transition-all shadow-lg hover:bg-purple-750 cursor-pointer"
            >
        ...
        {/* Title */}
        <h2 className="text-2xl sm:text-3xl font-black text-white tracking-tight leading-tight mb-2 uppercase">
          {movie.title}
        </h2>

        {/* Metadata row */}
        <div className="flex flex-wrap items-center gap-4 text-xs text-zinc-400 mb-5 font-semibold border-b border-white/5 pb-4.5">
        ...
        <div className="grid grid-cols-1 gap-4 border-t border-white/5 pt-5 mb-2">
          ...
          <div className="flex gap-3 items-start">
            ...
            <div>
              ...
              <span className="text-zinc-300 text-xs leading-relaxed">{movie.cast.join(', ')}</span>
            </div>
          </div>
        </div>
        ...
        {showTrailer && movie.trailerUrl && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-lg">
            ...
            <div className="relative w-full max-w-2xl aspect-video rounded-2xl overflow-hidden border border-white/10 bg-black z-50 shadow-2xl">
              <button
